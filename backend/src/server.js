...
async function migrate() {
  ...
  await run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS "blocked" BOOLEAN DEFAULT false`);
  await run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS "resetToken" TEXT`);
  await run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS "resetTokenExpires" TIMESTAMP`);
  await run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS "abandonedCartEmailSentAt" TIMESTAMP`);
  await run(`
