...
export default function WhatsAppOrderButton({ orderId, total, className = '' }) {
  ...
  const mensagem = [
    'Olá, Atlanta Sports! 🏆',
    'Acabei de fazer um pedido na loja e gostaria de finalizar pelo WhatsApp.',
    '',
    codigo && `*Código do pedido:* ${codigo}`,
    totalFormatado && `*Total:* ${totalFormatado}`,
    '',
  ...
  return (
    <a
      ...
      className={`flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl px-6 py-3 transition-colors ${className}`}
    >
